...
/* Valid opers */
enum opers{ DET=1,ADD,SUB,MUL,TRN,INV,TYPE,EXIT };
...
void matrixcalc_menu(void)
{
    ...
    if(INVALID != oper_valid(matrixcalc_oper))
    {
        ...
        scanf("%d", &n);
        if(n<2){ printf("\n Invalid order.Please renter the order.");
                  goto up;
            }
        operands.matrix_1 = alloc_input_matrix(operands.matrix_1,n);
        if(((matrixcalc_oper != DET )&&(matrixcalc_oper != TRN ) && (matrixcalc_oper != TYPE)&&(matrixcalc_oper != INV))){
    ...
    switch(matrixcalc_oper)
    {
        ...
        case INV:
            ...
            break;
        case TYPE: detertype(operands.matrix_1,n);
                   free_matrix_structure(&operands);
                   break;
